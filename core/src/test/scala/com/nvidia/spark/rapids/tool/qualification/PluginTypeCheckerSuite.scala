...
class PluginTypeCheckerSuite extends FunSuite with Logging {
  ...
  val platformSpeedupEntries: Seq[(Platform, Map[String, Double])] = Seq(
    (new OnPremPlatform(None, None), Map("UnionExec" -> 3.0, "Ceil" -> 4.0)),
    (new DataprocPlatform(None, None), Map("UnionExec" -> 4.88, "Ceil" -> 4.88)),
    (new EmrPlatform(None, None), Map("UnionExec" -> 2.07, "Ceil" -> 2.07)),
    (new DatabricksAwsPlatform(None, None), Map("UnionExec" -> 2.45, "Ceil" -> 2.45)),
    (new DatabricksAzurePlatform(None, None),
      Map("UnionExec" -> 2.73, "Ceil" -> 2.73)),
    (new DataprocServerlessPlatform(None, None),
      Map("WindowExec" -> 4.25, "Ceil" -> 4.25)),
    (new DataprocPlatform(None, None), Map("UnionExec" -> 4.16, "Ceil" -> 4.16)),
    (new DataprocGkePlatform(None, None), Map("WindowExec" -> 3.65, "Ceil" -> 3.65)),
    (new DataprocGkePlatform(None, None), Map("WindowExec" -> 3.74, "Ceil" -> 3.74)),
    (new EmrPlatform(None, None), Map("UnionExec" -> 2.59, "Ceil" -> 2.59))
  )
