  ...
  test("No recommendations generated for CPU eventLogs when plugin set to the wrong values") {
    ...
    val expectedResults =
      s"""|Cannot recommend properties. See Comments.
          |
          |Comments:
          |- AutoTuner recommendations only support eventlogs generated by Spark applications utilizing RAPIDS Plugin
          |- RAPIDS Plugin jar is missing in "spark.plugins". Please refer to https://nvidia.github.io/spark-rapids/Getting-Started
          |""".stripMargin
    // scalastyle:on line.size.limit
    assert(expectedResults == autoTunerOutput)
  }
