  ...
  test("No recommendations generated for CPU eventLogs when plugin not set") {
    ...
    val expectedResults =
      s"""|Cannot recommend properties. See Comments.
          |
          |Comments:
          |- AutoTuner recommendations only support eventlogs generated by Spark applications
          |  running on GPU hardware with Spark RAPIDS Plugin enabled.
          |- The Plugin jar is missing. Please refer to https://github.com/NVIDIA/spark-rapids/blob/branch-23.04/docs/get-started/getting-started.md
          |  to get started on using the GPU plugin then enable Spark RAPIDS  plugin by setting spark.rapids.sql.enabled=true
          |""".stripMargin
    // scalastyle:on line.size.limit
    assert(expectedResults == autoTunerOutput)
  }
  ...
  test("No recommendations generated for CPU eventLogs when plugin not enabled") {
    ...
    val expectedResults =
      s"""|Cannot recommend properties. See Comments.
          ...
          |- AutoTuner recommendations only support eventlogs generated by Spark applications
          |  running on GPU hardware with Spark RAPIDS Plugin enabled.
          |- Please enable Spark RAPIDS plugin by setting spark.rapids.sql.enabled=true
          |""".stripMargin
    // scalastyle:on line.size.limit
    assert(expectedResults == autoTunerOutput)
  }
