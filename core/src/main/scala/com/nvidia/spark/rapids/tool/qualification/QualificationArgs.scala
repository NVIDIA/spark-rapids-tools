  ...
  val maxEventLogSize: ScallopOption[String] =
    opt[String](required = false,
      descr = "Process only application event logs whose size is less than or equal to the size " +
        "specified. Valid units of size are b(bytes),k(kilobytes),m(metabytes),g(gigabytes). " +
        "If no units are specified, the size is assumed to be mb. Note, this does not support " +
