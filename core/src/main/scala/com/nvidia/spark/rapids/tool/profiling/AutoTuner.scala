...
class AutoTuner(
  ...
  def getRecommendedProperties(
    ...
    if (!isPluginLoaded || !rapidsEnabled) {
      appendComment("AutoTuner recommendations only support " +
          "eventlogs generated by Spark applications\n  running on GPU hardware with Spark " +
          "RAPIDS Plugin enabled.")
      if (!isPluginLoaded) {
        appendComment("The Plugin jar is missing. Please refer to https://github.com/NVIDIA/" +
            "spark-rapids/blob/branch-23.04/docs/get-started/getting-started.md\n  to get " +
            "started on using the GPU plugin then enable Spark RAPIDS  plugin by setting " +
            "spark.rapids.sql.enabled=true")
      }
      else if (!rapidsEnabled) {
        appendComment("Please enable Spark RAPIDS plugin by setting " +
            "spark.rapids.sql.enabled=true")
      }
    } else {
