...
class AutoTuner(
  ...
  def getRecommendedProperties(
    ...
    if (!isPluginLoaded || !rapidsEnabled) {
      ...
      if (!isPluginLoaded) {
        appendComment("The Plugin jar is missing from \"spark.plugins\". Please refer " +
            "to https://nvidia.github.io/spark-rapids/Getting-Started\n  to get started on " +
            "using the GPU plugin then enable Spark RAPIDS plugin by setting " +
