...
case class DataWritingCommandExecParser(
  ...
  def checkCompressionSupported(cmdWrapper: DataWritingCmdWrapper): Boolean = {
    ...
    writeOpMeta.dataFormat() match {
      ...
      case _ => true // Parquet/ORC are only handled. Anything else we assume is supported.
    }
  }
...
// The purpose is to reuse the code between different components.
...
object DataWritingCommandExecParser {
  ...
  // A set of the logical commands mapped to the physical write command,
  // which has an entry in the speedupSheet:
  // — InsertIntoHadoopFsRelationCommand is a logical command, which does not have an entry in the
  //   supported execs file.
  ...
  // This is used to extract the format and then check whether the
  ...
  def getWriteFormatString(node: String): String = {
    ...
    // write format is the fourth parameter.
  ...
  def getWriteOpMetaFromNode(node: SparkPlanGraphNode): WriteOperationMetadataTrait = {
    ...
    buildWriteOpMetaInternal(node, cmdWrapper)
  }

  def buildWriteOpMetaInternal(
      node: SparkPlanGraphNode,
      cmdWrapper: Option[DataWritingCmdWrapper]): WriteOperationMetadataTrait = {
    // Process the command wrapper to extract metadata
    cmdWrapper match {
