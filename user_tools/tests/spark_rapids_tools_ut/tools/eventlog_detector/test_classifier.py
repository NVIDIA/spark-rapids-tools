...
class TestRapidsConfigMarkers:
    ...
    def test_plain_spark_props_have_no_rapids_markers(self):
        assert has_rapids_conf_markers({"spark.master": "local"}) is False
    ...
    def test_disabled_rapids_plugin_still_counts_as_marker(self):
        ...
        assert has_rapids_conf_markers(props) is True

    def test_rapids_enabled_key_alone_counts_as_marker(self):
        assert has_rapids_conf_markers({"spark.rapids.sql.enabled": "true"}) is True

    @pytest.mark.parametrize("props", [
        {"spark.plugins": "com.nvidia.spark.SQLPlugin"},
        {"spark.plugins": "foo,com.nvidia.spark.SQLPlugin"},
        {
            "spark.plugins": "com.nvidia.spark.SQLPlugin",
    ...
    def test_lookalike_plugin_class_does_not_count_as_marker(self):
        assert has_rapids_conf_markers({"spark.plugins": "foo.nvidia.spark.SQLPlugin"}) is False
