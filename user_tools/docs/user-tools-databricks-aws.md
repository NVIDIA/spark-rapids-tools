...
- Set the configuration settings and credentials of the Databricks CLI:
  ...
  - Verify that the access credentials are stored in the file `~/.databrickscfg` on Unix, Linux, or macOS, or `%USERPROFILE%\.databrickscfg` on Windows.
...
- For AWS CLI, some environment variables can be set and picked by the RAPIDS-user tools such as: `AWS_PROFILE`, `AWS_DEFAULT_REGION` and `AWS_CONFIG_FILE`. See the full list of variables in [aws-cli-configure-envvars](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-envvars.html).
...
2. A user sets up his development machine:
   ...
   2. installs Databricks CLI and configures the profile and the credentials to make sure the
      ...
      or `%USERPROFILE%\.databrickscfg` on Windows.
